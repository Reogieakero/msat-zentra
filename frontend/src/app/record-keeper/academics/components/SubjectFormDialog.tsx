...
export function SubjectFormDialog({ open, subject, onOpenChange, onSave }: Props) {
  ...
  const [grades, setGrades] = React.useState<GradeLevel[]>([subject?.gradeLevel ?? 7]);
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      // grade, so the same code may live in several grades).
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={stepper.dialog}>
        <DialogHeader>
          ...
          <DialogDescription>
            Configure a subject for grades 7–10. Subject code must be unique per grade.
          </DialogDescription>
        </DialogHeader>
                  ...
                  {isEdit ? (
                    ...
                    <>
                      ...
                      <span className={styles.hint}>
                        Select one or more grades — a subject row is created per grade.
                      </span>
                    </>
                  )}
                </div>
