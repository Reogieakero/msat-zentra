...
import { fetchRegistrarOverview, type RegistrarGradeCountRow } from "./overview-data";
...
function interpretFinalGradeApproval(
  ...
  if (total === 0) return "No report-card entries are on record for the G11–12 band yet.";
...
function interpretSf10(released: number, available: number, total: number): string {
  if (total === 0) return "No SF10 records are on file for the G11–12 band yet.";
  let text = `${released} of ${total} SF10 records released`;
  if (available > 0) text += `, ${available} on hand`;
  return text + ".";
}

function interpretGradeCounts(rows: RegistrarGradeCountRow[], noun: string): string {
  if (rows.length === 0 || rows.every((r) => r.count === 0)) {
    return `No ${noun.toLowerCase()} registered for the G11–12 band this term.`;
  }
