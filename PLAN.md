...
- **Confirmed `action_type` extensions (non-exhaustive, system-generated by service layer):**
  - ADM: `adm_principal_approve` (Principal final-sign).
  - Calendar (Principal scope): `school_year_create`, `term_create`,
    `school_year_set_active`, `school_year_edit`.
  - Awards / Reports: `honor_roll_mark_awarded`, `report_refresh`.
  - Self-service: `principal_profile_change`, `principal_password_change`.
- Every sensitive write path across confidential tables writes a row with
  user_id / actor_role / source_table / source_id / reason / old_value / new_value.
- `actor_role` is derived from `users.role` at write time (joined server-side for display; not a separate FK to avoid drift).
- Principal has school-wide read access to the log. Drill-down from an audit row
  into a confidential `source_table` (anecdotal_records, health_records,
  home_visitation_records, adm_learner_profiles) returns **status-only** — hidden
  clinical columns are never exposed via the audit drill-down (O1).
- `old_value` / `new_value` JSON is visible in the audit log (system events, not
  clinical write-ups); a server-side payload size cap prevents oversized rows in UI.
